...
open class GenPagesTemplateLongWaterflowNestedLongWaterflowNested : BasePage {
    ...
    open var swiperList: UTSArray<SwiperViewItem6> by `$data`;
    ...
    open var swiperTabsRect: UTSArray<SwiperTabsItem5> by `$data`;
    open var nestedScrollChildId: String by `$data`;
    @Suppress("USELESS_CAST")
    override fun data(): Map<String, Any?> {
        return utsMapOf("swiperList" to utsArrayOf<SwiperViewItem6>(SwiperViewItem6(type = "UpdatedDate", name = "最新上架", id = "list-id-1", preload = true), SwiperViewItem6(type = "FreeHot", name = "免费热榜", id = "list-id-2", preload = false), SwiperViewItem6(type = "PaymentHot", name = "付费热榜", id = "list-id-3", preload = false), SwiperViewItem6(type = "HotList", name = "热门总榜", id = "list-id-4", preload = false)), "swiperIndex" to 0, "pageScrollView" to null as UniElement?, "headerHeight" to 0, "animationFinishIndex" to 0, "tabScrollView" to null as UniElement?, "indicatorNode" to null as UniElement?, "swiperWidth" to 0, "swiperTabsRect" to utsArrayOf<SwiperTabsItem5>(), "nestedScrollChildId" to "");
    }
    override fun `$initMethods`() {
        ...
        this.cacheTabItemsSize = fun() {
            ...
            tabs.forEach(fun(node){
                this.swiperTabsRect.push(SwiperTabsItem5(x = node.offsetLeft, w = node.offsetWidth));
            }
            );
        }
        ;
        ...
        this.updateTabIndicator = fun(current_index: Number, move_to_index: Number, percentage: Number) {
            ...
            val indicator_line_x = lerpNumber6(current_size.x, move_to_size.x, percentage);
            val indicator_line_w = lerpNumber6(current_size.w, move_to_size.w, percentage);
    ...
    companion object {
        ...
        var components: Map<String, CreateVueComponent> = utsMapOf("longPage" to GenPagesTemplateLongWaterflowNestedLongWaterflowPageClass);
    }
}
